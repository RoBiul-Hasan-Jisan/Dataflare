...
# The Next.js frontend runs on a different origin/port, so cookies need
# SameSite=None + Secure in production, and CORS needs to allow credentials.
...
# Allow the Next.js frontend (a different origin) to call this API with
...
@app.route('/')
def index():
    """API root - the UI lives in the separate Next.js frontend."""
    return jsonify({
        'name': 'DataFlare ML Studio API',
        'status': 'ok',
        'docs': '/api/test'
    })
...
@session_required
def session_status():
    """Lets the frontend know what's already loaded for this session."""
